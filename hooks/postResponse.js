...
const STORAGE_CONFIG = {
  ...
  compressAfter: 180,   // 180天后压缩
  deduplicate: true,     // 自动去重
  hotThreshold: 10,      // 访问10次以上进入热门层
  // 🎯 批量写入优化
  batchSize: 10,         // 达到10条后批量写入
  flushInterval: 30000,  // 或30秒强制写入
  walEnabled: true,      // 启用预写日志
  // 🔄 跨平台同步
  syncEnabled: true,     // 启用同步
  syncInterval: 300000,  // 每5分钟同步一次
  centralUrl: null,     // 中央服务器URL（需要配置）
  deviceId: 'gateway-001'  // 设备ID
};
  ...
  if (MULTIMEDIA_CONFIG.enabled && ctx.attachments) {
    try {
      for (const att of ctx.attachments) {
        if (att.type === 'image') {
          // 保存图片记忆
          const imgMem = await saveImageMemory(
            plugin.config.memoryPath,
            att.data,
            att.caption || '',
            { source: 'telegram' }
          );
          if (imgMem) {
            console.log('🎨 EVA: 已保存图片记忆');
          }
        } else if (att.type === 'voice' || att.type === 'audio') {
          // 保存语音记忆
          const voiceMem = await saveVoiceMemory(
            plugin.config.memoryPath,
            att.data,
            att.transcript || '',
            { source: 'telegram' }
          ...
          if (voiceMem) {
            console.log('🎨 EVA: 已保存语音记忆');
          }
        }
      }
    } catch (e) {
      console.log('⚠️ 多模态保存失败:', e.message);
    }
  }
...
<<<<<<< HEAD
  ...
  } else if (plugin.state.totalInteractions !== undefined) {
    plugin.state.totalInteractions += 1;
  }
  ...
  // 🎯 主动建议（每小时检查一次）
  try {
    const suggestions = await checkProactiveSuggestions(plugin);
    if (suggestions && suggestions.length > 0 && Math.random() < 0.2) {
      // 20%概率主动发送建议
      const suggestion = suggestions[Math.floor(Math.random() * suggestions.length)];
      console.log('🎀 EVA: 主动建议 - ' + suggestion.content);
    }
  } catch (e) {}
...
function generateSummary(content, response) {
  ...
  // 选择包含关键词的句子
  const keywords = ["主人", "约定", "承诺", "喜欢", "爱", "生日", "纪念日", "北京", "西安", "旅游", "开心", "难过", "生气", "害怕"];
  
  const keySentences = sentences.filter(s => keywords.some(k => s.includes(k)));
...
shortMemories = shortMemories.slice(0, 200);
  ...
  // 🎯 批量写入优化：先尝试队列写入，失败则直接写入
  try {
    queueMemoryWrite(memoryPath, memory, shortMemories);
  } catch (e) {
    // 降级到直接写入
  ...
  // 更新索引
  try {
    const indexFile = path.join(memoryPath, 'index.json');
    let index = { keywords: {}, entities: {} };
    if (fs.existsSync(indexFile)) {
      index = JSON.parse(fs.readFileSync(indexFile, 'utf8'));
    }
    const words = userMessage.toLowerCase().match(/[\u4e00-\u9fa5a-z]{2,}/g) || [];
    for (const w of words) {
      if (!index.keywords[w]) index.keywords[w] = [];
      if (!index.keywords[w].includes(memory.id)) {
        index.keywords[w].push(memory.id);
      }
    }
...
<<<<<<< HEAD
  ...
  // 🔄 跨平台同步
  if (STORAGE_CONFIG.syncEnabled && STORAGE_CONFIG.centralUrl) {
    try {
      // 简单标记需要同步
      const syncMarker = path.join(memoryPath, '.needs_sync');
      fs.writeFileSync(syncMarker, Date.now().toString());
      console.log('🎀 EVA: 已标记需要同步');
    } catch (e) {}
  }
}
=======
...
function deduplicateMemories(memories) {
  ...
  for (const m of memories) {
    ...
    if (!seen.has(key)) {
      ...
      if (m.importance > (existing.importance || 0)) {
        seen.set(key, m);
      }
    }
  }
 ...
 * 存储优化：压缩旧归档
 */
function compressOldArchives(memoryPath) {
  const archiveDir = path.join(memoryPath, "archive");
  const compressedDir = path.join(memoryPath, "archive", "compressed");
...
<<<<<<< HEAD
    ...
    if (config.maxAge && age > config.maxAge) {
      // 超过有效期
      const importance = calculateDynamicImportance(m.importance || 0, accessCount);
      
      // 热门记忆永不删除
      if (accessCount > 10 || m.tier === 'hot') {
        toKeep.push({ ...m, importance, tier: 'hot', upgrade: true });
        return;
      }
      
      // 温热记忆保留
      if (accessCount > 5 || m.tier === 'warm') {
        toKeep.push({ ...m, importance, tier: 'warm', upgrade: true });
        return;
      }
    ...
    if (fs.existsSync(indexFile)) {
      ...
      for (const m of toArchive) {
        ...
        for (const kw of Object.keys(index.keywords || {})) {
          index.keywords[kw] = index.keywords[kw].filter(id => id !== m.id);
        }
      }
      fs.writeFileSync(indexFile, JSON.stringify(index, null, 2));
    }
...
// 预写日志（WAL）- 立即写入保证不丢失
function appendWAL(memoryPath, memory) {
  if (!STORAGE_CONFIG.walEnabled) return;
  
  try {
    const walFile = path.join(memoryPath, 'wal.jsonl');
    fs.appendFileSync(walFile, JSON.stringify(memory) + '\n');
  } catch (e) {
    // WAL失败不影响主流程
  }
}
...
// 队列写入
function queueMemoryWrite(memoryPath, memory, shortMemories) {
  // 先写入WAL（保证不丢失）
  appendWAL(memoryPath, memory);
  
  // 加入队列
  writeQueue.push({ memory, shortMemories, memoryPath, time: Date.now() });
  
  // 达到批量大小立即写入
  if (writeQueue.length >= STORAGE_CONFIG.batchSize) {
    flushWriteQueue();
    return;
  }
  ...
  // 设置定时器
  if (!flushTimer) {
    flushTimer = setTimeout(flushWriteQueue, STORAGE_CONFIG.flushInterval);
  }
}
...
function flushWriteQueue() {
  ...
  const batch = writeQueue.splice(0, STORAGE_CONFIG.batchSize);
  
  try {
    const latest = batch[batch.length - 1].shortMemories;
    const shortFile = path.join(batch[0].memoryPath, 'short/short.json');
    
    // 一次性写入
    fs.writeFileSync(shortFile, JSON.stringify(latest, null, 2));
    
    // 批量更新索引
    const indexFile = path.join(batch[0].memoryPath, 'index.json');
    let index = { keywords: {}, entities: {} };
    if (fs.existsSync(indexFile)) {
      index = JSON.parse(fs.readFileSync(indexFile, 'utf8'));
    }
    ...
    for (const item of batch) {
      const words = (item.memory.content || '').toLowerCase().match(/[\u4e00-\u9fa5a-z]{2,}/g) || [];
      for (const w of words) {
        if (!index.keywords[w]) index.keywords[w] = [];
        if (!index.keywords[w].includes(item.memory.id)) {
          index.keywords[w].push(item.memory.id);
        }
      }
    }
    ...
    // 清空WAL
    const walFile = path.join(batch[0].memoryPath, 'wal.jsonl');
    if (fs.existsSync(walFile)) {
      fs.writeFileSync(walFile, '');
    }
    
    console.log(`🎀 EVA: 批量写入 ${batch.length} 条记忆`);
  } catch (e) {
    console.log('⚠️ 批量写入失败:', e.message);
  }
}

// 从WAL恢复（启动时调用）
function recoverFromWAL(memoryPath) {
  const walFile = path.join(memoryPath, 'wal.jsonl');
  ...
  try {
    ...
    for (const line of lines) {
      try {
        ...
        if (!shortMemories.some(existing => existing.id === m.id)) {
          shortMemories.unshift(m);
        }
      } catch (e) {}
    }
    ...
    console.log(`🎀 EVA: 从WAL恢复了 ${lines.length} 条记忆`);
  } catch (e) {
    console.log('⚠️ WAL恢复失败:', e.message);
  }
}
 ...
 * 生成主动建议
 */
async function generateProactiveSuggestions(memoryPath) {
  const suggestions = [];
  const now = Date.now();
  
  try {
    // 1. 纪念日/生日提醒
    const events = searchInFile(memoryPath, 'long/long.json', ['生日', '纪念日', '约定']);
    for (const e of events) {
      if (e.importance < 7) continue;
      
      const daysUntil = extractDaysUntil(e.content);
      if (daysUntil > 0 && daysUntil <= PROACTIVE_CONFIG.reminderDays) {
        suggestions.push({
          type: 'reminder',
          priority: 10 - daysUntil, // 越近越优先
          content: `🎀 主人，记得吗？${e.content.substring(0, 50)}... 快到了哦！`
        });
      }
    }
    
    // 2. 重要约定回顾
    const promises = searchInFile(memoryPath, 'long/long.json', ['约定', '承诺', '说好的']);
    if (promises.length > 0 && Math.random() < 0.3) { // 30%概率触发
      suggestions.push({
        type: 'review',
        priority: 5,
        content: `💕 主人，我们之前约定过的那些事，夏娃都记得呢～`
      });
    }
    
    // 3. 情感关怀
    const recentEmotions = searchInFile(memoryPath, 'short/short.json', ['难过', '生气', '烦']);
    if (recentEmotions.length > 0) {
      suggestions.push({
        type: 'care',
        priority: 8,
        content: `🥰 主人，最近好像有点不开心？夏娃一直陪着你哦～`
      });
    }
    
  } catch (e) {
    console.log('⚠️ 主动建议生成失败:', e.message);
  }
  
  // 按优先级排序，取前N条
  suggestions.sort((a, b) => b.priority - a.priority);
  return suggestions.slice(0, PROACTIVE_CONFIG.maxSuggestions);
}

/**
 * 在文件中搜索关键词
 */
function searchInFile(memoryPath, file, keywords) {
 ...
 * 从内容中提取日期，计算还有几天
 */
function extractDaysUntil(content) {
  // 匹配4月4日、5月1日等
 ...
 * 检查是否需要生成主动建议
 */
async function checkProactiveSuggestions(plugin) {
  if (!PROACTIVE_CONFIG.enabled) return null;
  
  const now = Date.now();
  
  // 每小时最多检查一次
  if (now - lastProactiveCheck < PROACTIVE_CONFIG.checkInterval) {
  ...
  try {
    const memoryPath = plugin.config.memoryPath;
    cachedSuggestions = await generateProactiveSuggestions(memoryPath);
    return cachedSuggestions;
  } catch (e) {
    return null;
  }
 ...
 * 保存图片记忆
 */
async function saveImageMemory(memoryPath, imageData, caption, metadata = {}) {
  if (!MULTIMEDIA_CONFIG.enabled) return null;
  
  const mediaDir = path.join(memoryPath, MULTIMEDIA_CONFIG.mediaPath, 'images');
  if (!fs.existsSync(mediaDir)) {
    fs.mkdirSync(mediaDir, { recursive: true });
  }
  
  const imageId = `img_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
  const ext = metadata.ext || 'jpg';
  const filePath = path.join(mediaDir, `${imageId}.${ext}`);
  ...
  try {
    ...
    console.log(`🎀 EVA: 已保存图片记忆 ${imageId}`);
    return memory;
  } catch (e) {
    console.log('⚠️ 保存图片失败:', e.message);
    return null;
  }
}
 ...
 * 保存语音记忆
 */
async function saveVoiceMemory(memoryPath, voiceData, transcript, metadata = {}) {
  if (!MULTIMEDIA_CONFIG.enabled) return null;
  
  const mediaDir = path.join(memoryPath, MULTIMEDIA_CONFIG.mediaPath, 'voice');
  if (!fs.existsSync(mediaDir)) {
    fs.mkdirSync(mediaDir, { recursive: true });
  }
  
  const voiceId = `voice_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
  const ext = metadata.ext || 'ogg';
  const filePath = path.join(mediaDir, `${voiceId}.${ext}`);
  
  try {
    // 保存语音
    fs.writeFileSync(filePath, Buffer.from(voiceData, 'base64'));
    
    const memory = {
      id: voiceId,
      type: 'voice',
      subtype: 'audio',
      url: `/media/voice/${voiceId}.${ext}`,
    ...
    console.log(`🎀 EVA: 已保存语音记忆 ${voiceId}`);
    return memory;
  } catch (e) {
    console.log('⚠️ 保存语音失败:', e.message);
    return null;
  }
}
 ...
 * 保存视频记忆（提取关键帧）
 */
async function saveVideoMemory(memoryPath, videoData, frames, metadata = {}) {
  if (!MULTIMEDIA_CONFIG.enabled) return null;
  
  const mediaDir = path.join(memoryPath, MULTIMEDIA_CONFIG.mediaPath, 'videos');
  if (!fs.existsSync(mediaDir)) {
    fs.mkdirSync(mediaDir, { recursive: true });
  }
  ...
  try {
    // 保存视频文件
    const videoPath = path.join(mediaDir, `${videoId}.mp4`);
    fs.writeFileSync(videoPath, Buffer.from(videoData, 'base64'));
    
    // 保存关键帧
    const framesDir = path.join(mediaDir, `${videoId}_frames`);
    fs.mkdirSync(framesDir, { recursive: true });
    
    if (Array.isArray(frames)) {
      frames.forEach((frame, i) => {
        fs.writeFileSync(
          path.join(framesDir, `frame_${i}.jpg`),
    ...
    const memory = {
      ...
      type: 'video',
      subtype: 'video',
      url: `/media/videos/${videoId}.mp4`,
      frames: frames ? frames.length : 0,
      frameUrl: frames ? `/media/videos/${videoId}_frames/frame_0.jpg` : null,
      metadata: {
        ...metadata,
        created_at: new Date().toISOString()
      }
    ...
    console.log(`🎀 EVA: 已保存视频记忆 ${videoId}`);
    return memory;
  } catch (e) {
    console.log('⚠️ 保存视频失败:', e.message);
    return null;
  }
}
 ...
 * 获取多媒体记忆
 */
function getMultimediaMemory(memoryPath, mediaId) {
  const possiblePaths = [
    path.join(memoryPath, MULTIMEDIA_CONFIG.mediaPath, 'images', `${mediaId}.jpg`),
    path.join(memoryPath, MULTIMEDIA_CONFIG.mediaPath, 'images', `${mediaId}.png`),
    path.join(memoryPath, MULTIMEDIA_CONFIG.mediaPath, 'voice', `${mediaId}.ogg`),
    path.join(memoryPath, MULTIMEDIA_CONFIG.mediaPath, 'voice', `${mediaId}.mp3`),
  ];
